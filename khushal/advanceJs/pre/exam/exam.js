...
//     let genderdata =
//       product.gender.toLowerCase() === userInputGender.toLowerCase();
//     return genderdata;
//   });
//   return data;
// }
...
//     (product) => product.gender === userGender
//   );
...
function getProductDescriptions(category, brand) {
  ...
  let matchedProducts = products.filter(
    (product) => product.category.includes(category) && product.brand === brand
  );

  // Extract descriptions from matched products
  const descriptions = matchedProducts.map((product) => product.description);
  console.log(
    "🚀 ~ file: 09_task.js:56 ~ getProductDescriptions ~ descriptions:",
    descriptions
  );
